...
import { Sparkles, ArrowRight, Mail, Lock, User, Phone } from 'lucide-react';
import useAuthStore from '../../store/authStore';

export default function CustomerSignupPage() {
  const [form, setForm] = useState({ name: '', email: '', password: '', phone: '' });
  const { customerSignup, loading, error } = useAuthStore();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const success = await customerSignup(form);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 relative overflow-hidden flex items-center justify-center px-4 sm:px-6 py-6">
      ...
      <div className="relative z-10 w-full max-w-md">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="text-center mb-8">
          ...
          <p className="text-slate-400 mt-2 text-sm">Sign up to book tables & track reservations</p>
        </motion.div>
        ...
        <form onSubmit={handleSubmit} className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 space-y-4">
          ...
          <div>
            <label className="block text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Phone <span className="text-slate-600">(optional)</span></label>
            <div className="relative">
        ...
        <p className="text-center text-sm text-slate-500 mt-6">
          ...
          <Link to="/customer/login" className="text-amber-400 font-semibold hover:text-amber-300 transition-colors">Sign In</Link>
        </p>
